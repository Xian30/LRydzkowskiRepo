...
#date of the job in api format    
current_date2 = f'{current_date.year}-{miesiac}-{dzien}'

#endpoint
endpoint_arrival_hist = f'https://app.goflightlabs.com/historical/{current_date2}?'

#api response
api_result_hist = requests.get(endpoint_arrival_hist, params)
...
#creating dataframe
dane_arrival = pd.DataFrame(data = zip(flight_type, status, departure_iataCode, departure_icaoCode\
...
#setting data types    
dane_arrival = dane_arrival.astype(var_names_types)   

#modyfing column values 
dane_arrival["arrival_iataCode"] = dane_arrival["arrival_iataCode"].map(lambda x: x.upper())
dane_arrival["departure_iataCode"] = dane_arrival["departure_iataCode"].map(lambda x: x.upper())

#saving the data in parquet format
dane_arrival.to_parquet(f"s3://lrpostgradudeflights/arrivals/arrival_{current_date2}.parquet")
